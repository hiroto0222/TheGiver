...
var key_combos := ["w", "w", "a", "s", "w", "d", "d", "a", "w", "s", "d"]
var key_combo_success_pitches := []
var key_combo_idx: int = 0
var key_letter_dist := 800
...
func handle_successful_key_press() -> void:
	...
	var rand_x: float = [randi_range(-200, -50), randi_range(50, 200)].pick_random()
	var rand_y: float = [randi_range(-200, -50), randi_range(50, 200)].pick_random()
